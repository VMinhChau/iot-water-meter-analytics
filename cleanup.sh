...
if [ -z "$KAFKA_CONTAINER" ]; then
  echo "⚠️ No Kafka BROKER container found."
else
  echo "🗑️ Deleting Kafka topic 'water-meter-readings' from container: $KAFKA_CONTAINER"
  docker exec "$KAFKA_CONTAINER" \
    kafka-topics --bootstrap-server localhost:9092 \
    --delete --topic water-meter-readings || true
fi

echo "🗑️ Deleting old Elasticsearch index..."
curl -X DELETE "localhost:9200/water-meter-index" || true

echo "🗑️ Deleting old HDFS data..."
